...
# === 解析手法の解説 ===
with st.expander("🔬 本アプリで使用している高度な統計・機械学習手法", expanded=False):
    st.markdown("""
#### 📊 テクニカル分析（無料機能）

本アプリの無料機能では、金融工学で標準的に用いられる以下のテクニカル指標を実装しています。

**移動平均分析（SMA: Simple Moving Average）** では、20日・50日・200日の3つの時間窓を用いて短期・中期・長期トレンドを同時に可視化します。ゴールデンクロス・デッドクロスの検出により、トレンド転換点を統計的に捕捉します。

**ボリンジャーバンド** は、20日移動平均を中心に ±2σ（標準偏差）の帯域を描画し、価格の統計的な分布範囲を可視化します。正規分布の仮定のもと、約95.4%の確率で価格がこの帯域内に収まることを意味します。

**RSI（Relative Strength Index）** は、14日間の上昇幅と下落幅の比率から算出される 0〜100 のオシレーター指標です。70以上で買われすぎ、30以下で売られすぎのシグナルとして機能します。

**MACD（Moving Average Convergence Divergence）** は、12日EMAと26日EMAの乖離を利用したモメンタム指標であり、シグナル線（9日EMA）との交差でエントリーポイントを検出します。

---

#### 🧠 AI予測エンジン（Pro機能）

Pro機能では、異なる学習アルゴリズムに基づく **3つの予測モデル** を構築し、それらを統計的に最適な重みで統合する **アンサンブル学習** を実装しています。

**① LSTM（Long Short-Term Memory）ニューラルネットワーク**

再帰型ニューラルネットワーク（RNN）の発展形であるLSTMを採用しています。通常のRNNが持つ勾配消失問題を、忘却ゲート・入力ゲート・出力ゲートの3つのゲート機構で解決し、長期的な時系列依存関係を学習可能にしています。

- アーキテクチャ: 2層LSTM（64→32ユニット）+ Dropout(0.2) + Dense(16) + 出力層
- 最適化: Adam（学習率 0.001）、Early Stopping（patience=5）
- 入力: 過去60日間の正規化済み終値系列（MinMaxScaler）
- 特徴: 非線形パターン、レジーム変化、長期記憶の捕捉に優位

**② Random Forest（ランダムフォレスト）**

バギング（Bootstrap Aggregating）に基づくアンサンブル決定木モデルです。200本の決定木を独立に構築し、その予測値の平均を最終出力とすることで、個別の木が持つ過学習リスクを大幅に低減します。

- 構成: 200本の決定木、最大深度20、最小分割サンプル数5
- 特徴: 外れ値への頑健性、特徴量重要度の算出、過学習耐性

**③ Gradient Boosting（勾配ブースティング）**

前の木の残差（予測誤差）を次の木が学習するという逐次的な最適化アルゴリズムです。損失関数の勾配方向に弱学習器を追加していくことで、予測精度を段階的に改善します。

- 構成: 200段のブースティング、最大深度5、学習率0.05、サブサンプル率0.8
- 特徴: 高い予測精度、残差パターンの逐次学習、正則化による汎化性能

---

#### ⚖️ アンサンブル統合手法

3つのモデルの予測値を **RMSE（二乗平均平方根誤差）の逆数** で重み付けして統合します。検証データでの予測精度が高いモデルほど大きな重みを与える、統計的に合理的な手法です。

**重み計算:** wi = (1/RMSEi) / Σ(1/RMSEj)

**95%信頼区間:** CI = 予測値 ± 1.96 × √(σ²pred + RMSE²avg)

この手法により、単一モデルでは捉えきれない多様な市場パターンを統合的に予測し、かつ予測の不確実性を定量的に評価することが可能です。
""")
